...
func gauges(myPlane : Aviatrix) {
    ...
    print(" ")
//    print("| Running:  | ✅")
      print("| Location:  | \(myPlane.current)")
...
func fly(myPlane : Aviatrix) {
    ...
    for (index, city) in destinations.enumerated() {
        let distance = myPlane.distanceTo(target : city, current : city)
    ...
    if response! >= 0 && response! < 4 {
        ...
        if fuelCheck(myPlane: myPlane, destination : desiredLocation) {
            ...
            print("🛬 You've arrived in \(myPlane.current)!")
...
var plane = Aviatrix(aviatrixAuthor: "Kab")

print("Welcome to the Aviatrix Flight System by \(plane.author)")
plane.start()

print("You're currently in \(plane.current)")
...
print(" ")
print("Thanks for flying with Kab airline!")
